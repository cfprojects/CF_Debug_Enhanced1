...
<cfif bFoundSQLQueries>
	<cftry>
		...
		<cfloop query="cfdebug_queries">
			...
			<!--- 20 JULY,06. AJAS MOHAMMED. JUST DELETE THE NEXT CFDUMP STATEMENT AND YOU WILL GET
			      THE DEFAULT FUNCITONALITY OF COLDFUSION I.E JUST SHWOING QUERY INSTEAD OF THE RESULTS IN
				  A GRID.Dump query contents --->
...
<code><b>#cfdebug_storedproc.name#</b> (Datasource=#cfdebug_storedproc.datasource#, Time=#Max(cfdebug_storedproc.executionTime, 0)#ms) in #cfdebug_storedproc.template# @ #TimeFormat(cfdebug_storedproc.timestamp, "HH:mm:ss.SSS")#</code><br />
    ...
    <tr>
        ...
        <td>
            ...
            <tr>
                ...
                <td>&nbsp;<code><cfif StructKeyExists(thisParam, "value")>#htmleditformat(thisParam.value)#</cfif> <cfif thisParam.type eq "out"><i style="color:red">#evaluate(thisParam.variable)#</i></cfif></code></td>
    ...
    <!--- Dump stored procedure contents Ben Forta tip . Ajas Aug 22,06--->
	<cfloop index="x" from=1 to="#arrayLen(cfdebug_storedproc.result)#">
	<cfset thisParam = #cfdebug_storedproc.result[cfdebug_storedproc.currentRow][x]#>
	<cfif StructKeyExists(thisParam, "name")>
		<br /><b> *** COLUMNS FROM THE STORED PROCEDURE *** SQL Name : <i style="color:red"><cfdump var="#cfdebug_storedproc.NAME#"></i> --- CFPROCRESULT Name : <i style="color:red">#thisParam.name#</i> --- 					    RESULTSET No :  <i style="color:red"><cfdump var="#thisParam.resultSet#"></i>
		</b>
		<br /><br />
		<cfdump var="#evaluate(thisParam.name)#">
		<br /><b>*****************************************************************************************
			  </b>
		<br /> <br />
	...
	<!--- End of Dump stored procedure contents Ben Forta tip . Ajas Aug 22,06--->
...
<!--- July 20 2009. Ajas added local VARIABLES --->
<!---<cftry>
<cfif IsDefined("variables")>
	<cfdump var="#getPageContext().getVariableScope()#" label="A list of all Local Variables on this page.">
</cfif>
<cfcatch type="Any"></cfcatch>
</cftry>--->
<!--- End of July 20 2009. Ajas added local VARIABLES --->
